...
Emergency commands are unhandled, and recording boundaries no longer emit the
old `trialStarted`, `trialCaptureEnded`, or `trialEnded` events. API clients must
use recording state/session metadata for session lifecycle and the persisted
trial ledger for pellet-trial accounting until a new session-aware API schema
replaces 0.9.22.
